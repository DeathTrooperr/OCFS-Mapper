...
<div class="relative flex justify-between items-center w-full mb-8 lg:mb-12 px-4">
    <!-- Progress Line -->
    ...
    {#each steps as step, i}
        ...
        >
            <div 
                class="w-8 h-8 lg:w-10 lg:h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300
            ...
            >
                ...
                {:else}
                    <span class="text-xs lg:text-sm font-bold">{i + 1}</span>
                {/if}
            </div>
            <span class="hidden md:block text-[10px] lg:text-xs font-bold uppercase tracking-wider transition-colors duration-300 whitespace-nowrap
